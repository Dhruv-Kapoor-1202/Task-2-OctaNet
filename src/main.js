...
function renderTasks(selectedList) {
  selectedList.tasks.forEach(task => {
    const taskElement = document.importNode(taskTemplate.content, true)  // true means it'll render everything inside the template and not just the top-level element
...
// Function to filter task cards based on search input
function filterTasks(searchTerm) {
  // const taskCards = document.querySelectorAll('.task');
  const taskCards = tasksContainer;
  taskCards.forEach(card => {
    const title = card.querySelector('.task label').textContent.toLowerCase();
